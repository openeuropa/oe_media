...
class CircaBcMiddleware {
    ...
    return function ($handler) {
      return function (RequestInterface $request, array $options) use ($handler) {
        ...
        if ($request->getMethod() === 'POST' &&  str_starts_with($path, '/circabc-ewpp/service/circabc/content/') && str_ends_with($path, '/translations/enhanced')) {
          ...
          // Allow tests to simulate a translation upload failure.
          if (\Drupal::state()->get('oe_media_circabc_mock_fail_translation')) {
            $response = new Response(500, [], 'Simulated translation upload failure');
            return new FulfilledPromise($response);
          }
  ...
  /**
   * Fills the upload template with document data from the state.
   *
   * @param array $upload
   *   The upload template.
  ...
  protected function fillUploadTemplate(array &$upload, array $document, string $uuid, bool $translations = FALSE): void {
    ...
    $pivot['title'][$language] = $document['title'] ? json_decode($document['title'], TRUE)[$language] : "";
    $pivot['description'][$language] = $document['description'] ? json_decode($document['description'], TRUE)[$language] : "";
    ...
    if (isset($document['publicationDate'])) {
      $date = \DateTime::createFromFormat('Y-m-d\TH:i:s.v\Z', $document['publicationDate']);
      $pivot['properties']['issue_date'] = $date->format('Y-m-d\TH:i\Z');
    }
    if (isset($document['creationDate'])) {
      $date = \DateTime::createFromFormat('Y-m-d\TH:i:s.v\Z', $document['creationDate']);
      $pivot['properties']['created'] = $date->format('Y-m-d\TH:i\Z');
    }
    ...
    if ($translations) {
      ...
      foreach ($document['translations'] as $langcode => $translation_data) {
        ...
        if (isset($translation_data['publicationDate'])) {
          $date = \DateTime::createFromFormat('Y-m-d\TH:i:s.v\Z', $translation_data['publicationDate']);
          $translation['properties']['issue_date'] = $date->format('Y-m-d\TH:i\Z');
        }
        if (isset($translation_data['creationDate'])) {
          $date = \DateTime::createFromFormat('Y-m-d\TH:i:s.v\Z', $translation_data['creationDate']);
          $translation['properties']['created'] = $date->format('Y-m-d\TH:i\Z');
        }
